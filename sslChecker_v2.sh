...
#if the checked domain redirects to a new domain, the script will analyze the new domain SSL certificate.

	F_URL=`curl -I $domain 2>/dev/null | grep "Location:" | awk -F "://" '{print $2}' | awk -F "/|:" '{print $1}'`

	if [ "$domain" != "$F_URL" ]; then

        	echo -e "${RED}Note:${YELLOW} The domain $domain redirects to a possible new domain $F_URL"
		echo -e "Analysis will be perfomed on $F_URL${WHITE}"
		echo ""
