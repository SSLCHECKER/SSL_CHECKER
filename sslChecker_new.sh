...
main(){
	...
	if [[ -z "$1" ]]; then
		...
		echo -e "${YELLOW}Total number of domains/subdomains:${WHITE} $NUM_SAN_D"
		...
		echo -e "${YELLOW}Total number of domains/subdomains:${WHITE} $NUM_SAN_F_URL"
	...
	elif [[ ! -z "$domain" && ! -z "$F_URL" && "$domain" != "$F_URL" ]] ; then
	...
	elif [[ "$domain" == "$F_URL" && ! -z "$F_URL" ]]; then
...
# Calculates the certificate age based on day, month and year. 
	...
	if [[ `echo openssl s_client -connect $i:443 -servername $i 2>&1 >/dev/null | grep "unable to load certificate"` ]]; then

		echo "The server supports v1"
		exit
...
#The script will strip "www." for whois check, as the command couldnot provide whois records for domains with "WWW." 
