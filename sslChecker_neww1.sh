...
main(){
	...
	F_URL=`curl -I -Ls $domain 2>/dev/null | grep "Location:"| tail -n 1 | awk -F "://" '{print $2}' | awk -F "/|:" '{print $1}'`
	...
	#NOCERT=$(echo | openssl s_client -connect $domain:443 -servername $domain 2>/dev/null | grep "errno=104")
