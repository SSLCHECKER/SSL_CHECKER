...
echo -e " Check SSL certificates to identify anomolies within certificates metadata\n"
...
main(){
	...
	echo "Usage: ./sslCheck4.sh [domain, without http/https] or [file]"
...
# CertAge_D/_M/_Y: this filed shows the validity of a certificate based on day, month and year. 
	...
	if [[ "$CertAge_Y" -gt 3 ]]; then 
		...
		echo "The certificate has a normal age (Accorrding to CA/Browser Forum, two years is the maximum validity period of new SSL certificates, starting on March 1, 2018)"
...
# Checks if the domain is listed as a value in the SAN field, if the domain is not listed, then this would indicate a mismatch.
  	...
  	if [ "$i" != "$ISSUEDTO" ]; then
		...
		D=`echo "$i" | cut -d "." -f1-3 | cut -d "" -f1-3`
...
# Checks if the domain whois record is “redacted” or “private”
	...
	PRIVATE=`whois $i | egrep -wi -m1 -o 'private|REDACTED|UNVERIFIED'`
...
# Checks the domain is whitelisted 
