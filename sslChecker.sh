...
#Description	: Checks SSL Certificates of domains, reveals malicious and benign domains 
#Date		: June 2019
#Version	: 0.1		                                                                                                                                                                        
...
echo -e " Check SSL certificates for malicious activities\n"
echo -e "./sslCheck.sh -h for help"
...
main(){
	...
	domain=$(echo $1 | grep -P "(?=^.{5,254}$)(^(?:(?!\d+\.)[a-za-z0-9_\-]{1,63}\.?)+(?:[a-za-z]{2,})$)")
	...
	echo "Usage: ./sslCheck4.sh [domain] or [file]"
	...
	elif [[ "$domain" && "$2" == "-l" ]]; then

		echo "---------------------"

    		echo -e "${YELLOW}List of domains sharing the same certificate of $domain:${WHITE}" 

		echo "---------------------"
		
		
		SAN=$(echo | openssl s_client -connect $1:443 -servername $1 2</dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5-)
		NUM_SAN=$(echo | openssl s_client -connect $1:443 -servername $1 2</dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5- | wc -l)
		
		echo -e "$SAN\n"
		
		echo "---------------------"

		echo -e "${YELLOW}Total number of domains:${WHITE} $NUM_SAN"

		echo "---------------------"

		exit

	elif [[ "$domain" && "$2" == "-s" ]]; then

		echo -e "---------------------"

    		echo -e "${YELLOW}List of Supported Ciphers for $domain${YELLOW}" 

		echo -e "${WHITE}---------------------"

		
		sslscan $domain | grep 'Accepted'
...
# Extracts the Fingerprint field which contains a unique identifier or hash sum (MD5, SHA-1 or SHA-256 and others) of the ASN.1 binary (DER) encoded certificates. 
	...
	ISSUEDT0=$(echo "$CERTINFO" | openssl x509 -noout -subject -nameopt multiline 2>/dev/null | grep commonName | grep -Eo '=.*' | cut -c 3-)
	...
	PROTOCOL=$(echo "$CERTINFO" | grep "Protocol  : " | awk -F ':' '{print $2}' )	
	...
	NUM_ALT_NAMES=$(echo | openssl s_client -connect $1:443 -servername $1 2</dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5- | wc -l )
...
# Calculates the certificate age based on day, month and year. 
	...
	if [[ -z "$CERTINFO" || "$CertAge_D" -eq 0  ]]; then
	...
	echo -e "${YELLOW}NUMBER OF DOMAINS SHARING THE SAME CERTIFICATES:${WHITE} $NUM_ALT_NAMES"
...
# Checks if the SSL certficate fingerprint is found in our SSL Blacklist lookup table.

	if true | grep "$FINGERPRINT" sha1_sslbl_unique.txt >/dev/null; then echo -e "The hash: $FINGERPRINT is listed in our SSL Blacklist lookup file ${RED}*${WHITE}"; fi

# Checks if the SSL certficate fingerprint is found in sslbl.abuse.ch Database which is updated every 5 minutes..

	if true ; MATCH_FINGERPRINT=`grep "$FINGERPRINT" /root/sslblacklist.csv`; then echo -e "The hash: $FINGERPRINT is listed in sslbl.abuse.ch Database ${RED}*${WHITE}"; echo "Record: $MATCH_FINGERPRINT${RED}*${WHITE}"; fi
...
# CertAge_D/_M/_Y: this filed shows the validity of a certificate based on day, month and year. 
	...
	if [[ "$CertAge_Y" -gt 3 ]]; then 
		...
		SHORT_AGE="The certificate has a very short age, maybe for testing or a suspicious activity ${RED}*${WHITE}"
		echo -e $SHORT_AGE
	else 
		
		echo "The certificate has a normal age"
...
# Checks if the domain is listed as a value in the SAN field, if the domain is not listed, then this would indicate a mismatch.
  	...
  	if [ "$i" != "$ISSUEDTO" ]; then
		...
		if [ -z "$ISSUEDTO"]; then
          		ISSUEDTO="-"
          		MISMATCH="Possible name mismatch of the domain and (ISSUED TO / SUBJECT) fields. Also, the domain is not listed as an Alternative Domain Name ${RED}*${WHITE}"
			echo -e $MISMATCH
...
# Checks if the domain whois record is “redacted” or “private”
	...
	PRIVATE=`whois $i | egrep -wi -m1 -o 'private|REDACTED'`
		
	if [[ "$PRIVATE" ]]; then
		
		echo -e "Some fields in Whois record is $PRIVATE ${RED}*${WHITE}"
...
# Checks if the domain is whitelisted 
	...
	if [ true | WHITELISTED=`grep "$i" whitelist.txt`]; then
		...
		echo ""
		echo -e "${RED}Very Suspicious, and Possibly Used in Malicious Activity. Requires a Block${WHITE}"
		VT_S $i
		...
		echo "Nothing Looks Suspicious with SSL Certificate Metadata"
...
VT_S(){	
		...
		echo ""
			...
			if [ "$VT_SCAN_M" -gt "0" ] || [ "$VT_SCAN_S" -gt "0" ]; then
				...
				echo -e "${RED}VT Observed the Domain in Malicious Activity, Requires to Be Blocked${WHITE}"
				echo "$i : $FINGERPRINT : $ISSUER" >> sha1_sslbl.txt 
				cat sha1_sslbl.txt | sort -u | uniq > sha1_sslbl_unique.txt
				whois_info $i
			else
...
whois_info(){
	...
	if grep -q 'No match\|Not found\|This TLD has no whois server\|No entries' whois.txt
	
	then
		echo "The Script Could not Provide Domain's Whois Record, Please Check it in Open Source Whois"
		echo ""
