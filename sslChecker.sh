...
echo -e "./sslChecker.sh -h for help"
...
main(){
	...
	domain=$(echo $1 | grep -P -i "(?=^.{5,254}$)(^(?:(?!\d+\.)[a-za-z0-9_\-]{1,63}\.?)+(?:[a-za-z]{2,})$)" )
	
#if the checked domain redirects to a new domain, the script will analyze the new domain (F_URL) SSL certificate.

	F_URL=`curl -I $domain 2>/dev/null | grep "Location:" | awk -F "://" '{print $2}' | awk -F "/|:" '{print $1}'`
	...
	if [ -f $1 ]; then
		...
		-s  --list supported suite ciphers. Usage: ./sslChecker.sh domain -s"
	...
	if [[ -z "$F_URL" && "$2" == "-l" ]]; then
		...
		SAN=$(echo | openssl s_client -connect $domain:443 -servername $domain 2>/dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5-)
		NUM_SAN=$(echo | openssl s_client -connect $domain:443 -servername $domain 2>/dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5- | wc -l)
	...
	if [[ ! -z "$F_URL" && "$2" == "-l" ]]; then
		...
		echo "---------------------"
    		...
    		echo -e "${YELLOW}List of domains sharing the same certificate of $F_URL:${WHITE}" 

		echo "---------------------"
		
		
		SAN=$(echo | openssl s_client -connect $F_URL:443 -servername $F_URL 2>/dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5-)
		NUM_SAN=$(echo | openssl s_client -connect $F_URL:443 -servername $F_URL 2>/dev/null | openssl x509 -noout -text | grep "Subject Alternative Name" -A2 | grep -Eo "DNS:[-a-zA-Z 0-9.*]*" | cut -c 5- | wc -l)
		...
		echo -e "---------------------"
    		...
    		echo -e "${YELLOW}List of Supported Ciphers for $F_URL${YELLOW}" 

		echo -e "${WHITE}---------------------"

		
		sslscan $F_URL | grep 'Accepted'
...
whois_info(){
	...
	then
		echo "The Script Could not Provide Domain's Whois Record, Please Check it in the Open Source Whois"
		echo ""
